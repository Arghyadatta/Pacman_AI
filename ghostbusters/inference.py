...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.legalPositions:
        ...
        allPossible[p] = self.beliefs[p] * emissionModel[Dist]
  ...
  def elapseTime(self, gameState):
    ...
    for old in self.legalPositions:
        newpd = self.getPositionDistribution(self.setGhostPosition(gameState, old))
        for new, probability in newpd.items():
            allPosible[new] += probability * self.beliefs[old]

    allPosible.normalize()
    self.beliefs = allPosible    
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    while self.numParticles > count:
        self.stateParticles.append(random.choice(self.legalPositions))
        count = count + 1
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance is None:
      self.stateParticles = [self.getJailPosition()] * self.numParticles
      return
    newBeliefs = util.Counter()
    total = 0
    for p in self.stateParticles:
    ...
    for p in range(self.numParticles):
      self.stateParticles.append(util.sample(newBeliefs))
  ...
  def elapseTime(self, gameState):
    ...
    #util.raiseNotDefined()
    newParticles = []
    for oldPos in self.stateParticles:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      newParticles.append(util.sample(newPosDist))
    self.stateParticles = newParticles
...
#    util.raiseNotDefined()
    ...
    for p in range(self.numParticles):
      ...
      self.stateParticles.append(particle)
  ...
  def elapseTime(self, gameState):
    ...
    """
    newParticles = []
    for oldParticle in self.stateParticles:
...
#    self.particles = newParticles
    ...
    for oldParticle in self.stateParticles:
      newParticle = ()
      ghostPos = setGhostPositions(gameState, oldParticle)
      for i in range(self.numGhosts):
        newPosDist = getPositionDistributionForGhost(ghostPos, i, self.ghostAgents[i])
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    jointBeliefs = util.Counter()

    for j in self.stateParticles:
        partial = 1
        for i in range(self.numGhosts):
            if noisyDistances[i] != None:
                trueDistance = util.manhattanDistance(j[i], pacmanPosition)
                partial = emissionModels[i][trueDistance] * partial
        jointBeliefs[j] += partial
    jointBeliefs.normalize()
    if jointBeliefs.totalCount() == 0:
        self.initializeParticles()
    else:
        self.stateParticles = []
        for p in range(self.numParticles):
            self.stateParticles.append(util.sample(jointBeliefs))
    for i in range(self.numGhosts):
        if noisyDistances[i] is None:
            newParticle = []
            for j in self.stateParticles:
                j = list(j)
                j[i] = self.getJailPosition(i)
                newParticle.append(tuple(j))
            self.stateParticles =newParticle
  
  def getBeliefDistribution(self):
    dist = util.Counter()
    for part in self.stateParticles: dist[part] += 1
    dist.normalize()
    return dist
