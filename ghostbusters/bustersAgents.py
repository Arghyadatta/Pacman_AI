...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for uncertainty in livingGhostPositionDistributions:
      ghost = uncertainty.argMax()
      positions[ghost] = self.distancer.getDistance(pacmanPosition, ghost)

    min_ghost_distance = min(positions, key=positions.get)
    nextPosition = util.Counter()

    for action in legal:
      pos = Actions.getSuccessor(pacmanPosition, action)
      nextPosition[action] = self.distancer.getDistance(pos, min_ghost_distance)

    action =  min(nextPosition, key=nextPosition.get)
    return action
